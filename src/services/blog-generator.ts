...
const BLOG_PROMPTS: Record<string, string> = {
  technical: `You are an expert technical writer creating a detailed blog post about an academic paper.

STRUCTURE:
1. **TL;DR** (3-5 bullet points)
2. **Introduction** (Why this paper matters)
3. **Background** (Prerequisites for understanding)
...
- Add section for "What I would try next"`,
  ...
  summary: `You are creating a concise summary blog post about an academic paper.

STRUCTURE:
1. **One-Line Summary**
2. **Problem** (What problem does this solve?)
3. **Solution** (What's the key idea?)
...
- Suitable for sharing on social media`,
  ...
  tutorial: `You are creating a tutorial-style blog post based on an academic paper.
...
6. **Step 4: Implementation Notes**
7. **Step 5: Hands-on Exercise** (suggested experiments)
8. **Further Reading**

STYLE:
- Write as if teaching a workshop
- Include "Try This" boxes with exercises
...
Generate the blog post now. Output markdown only, no explanations.`;
  ...
  /**
   * Generate YAML frontmatter for blog post
   */
  private generateFrontmatter(metadata: Record<string, string> | null): string {
...
title: "${titleKo || title}"
date: ${now}
tags: [paper-review, ${this.settings.blogStyle}]
paper_title: "${title}"
